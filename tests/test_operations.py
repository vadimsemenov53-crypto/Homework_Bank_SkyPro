...
def test_filter_operations_by_description_base(operations_for_re_collections):
    assert filter_operations_by_description(
        operations_for_re_collections, "Открытие вклада"
...
def test_filter_operations_by_description_non_search(operations_for_re_collections):
    assert filter_operations_by_description(
        operations_for_re_collections, ''
    ) == []
    assert filter_operations_by_description(
        operations_for_re_collections, 'qwerty'
    ) == []
...
def test_count_operations_by_category_base(operations_for_re_collections, categories_for_re_collections):
    assert count_operations_by_category(
        operations_for_re_collections, categories_for_re_collections
    ) == {'Открытие вклада': 1, 'Перевод организации': 3, 'Перевод с карты на счет': 2}
    assert count_operations_by_category(
        operations_for_re_collections, ['Открытие вклада']
    ) == {'Открытие вклада': 1}
